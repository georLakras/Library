<?php 	include('config.php');
		...
		$query= 'SELECT book.id AS book_id, book.title, book.isbn, book.release_year, book.available, writer.last_name, publisher.title AS publisher
				 FROM book 
				...
				<div class="page-header page-header-default">
					<div class="page-header-content">
						<div class="page-title">
							<h4><a href="index.php"><i class="icon-arrow-left52 position-left"></i></a> <span class="text-semibold">Books' Catalog. Here you can see a full list of all books in your library.</h4>
						</div>
					</div>
					...
					<div class="breadcrumb-line">
						<ul class="breadcrumb">
							...
							<li class="active"><a href="books_catalog.php">Books' Catalog</a></li>
								...
								<?php 
									if ($result->num_rows>0){
										while($row=$result->fetch_assoc()){
											$output  = '<tr>';				
											$output .= '<th scope="row">'.$row['book_id'].'</th>';									
											$output .= '<td>'.$row['title'].'</td>';
											$output .= '<td>'.$row['last_name'].'</td>';
											$output .= '<td>'.$row['publisher'].'</td>';										
											$output .= '<td>'.$row['release_year'].'</td>';								
											$output .= '<td>'.$row['isbn'].'</td>';
											if ($row['available']==1){
												$output .= '<td><i <span class="label label-success">Available</a></td>';
											}else{
												$output .= '<td><span class="label label-danger">No Available</span></td>';
											}
											$output .= '<td class="text-center">
															<ul class="icons-list">
																<li class="dropdown">
																	...
																	<ul class="dropdown-menu dropdown-menu-right">
																		<li><a href="book.php?id='.$row['book_id'].'"><i class="icon-info3"></i> View details</a></li>
																		<li><a href="edit_book.php?id='.$row['book_id'].'"><i class="icon-pen6"></i> Edit book</a></li>
																		<li><a href="delete_book.php?id='.$row['book_id'].'"><i class="icon-cross2"></i> Delete book</a></li>
																	</ul>
																</li>
															</ul>
														</td>';
